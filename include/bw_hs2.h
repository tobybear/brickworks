 ...
 *          <li>Now <code>bw_hs2_reset_state()</code> returns the initial output
 *              value.</li>
 ...
 *    #### bw_hs2_reset_state()
 *  ```>>> */
static inline float bw_hs2_reset_state(
 ...
 *  ```>>> */
static inline void bw_hs2_reset_state_multi(
	const bw_hs2_coeffs * BW_RESTRICT              coeffs,
	bw_hs2_state * BW_RESTRICT const * BW_RESTRICT state,
	const float *                                  x_0,
	float *                                        y_0,
	size_t                                         n_channels);
/*! <<<```
 ...
 *    #### bw_hs2_process1()
 *  ```>>> */
static inline float bw_hs2_process1(
	const bw_hs2_coeffs * BW_RESTRICT coeffs,
	bw_hs2_state * BW_RESTRICT        state,
	float                             x);
/*! <<<```
 ...
 *    #### bw_hs2_process()
 *  ```>>> */
static inline void bw_hs2_process(
	bw_hs2_coeffs * BW_RESTRICT coeffs,
	bw_hs2_state * BW_RESTRICT  state,
	const float *               x,
	float *                     y,
 ...
 *  ```>>> */
static inline void bw_hs2_process_multi(
	bw_hs2_coeffs * BW_RESTRICT                    coeffs,
	bw_hs2_state * BW_RESTRICT const * BW_RESTRICT state,
	const float * const *                          x,
	float * const *                                y,
 ...
 *    than or equal to that of `bw_hs2_coeffs`.
 *
 *    #### bw_hs2_state_is_valid()
 *  ```>>> */
...
#ifdef BW_DEBUG_DEEP
enum bw_hs2_coeffs_state {
	bw_hs2_coeffs_state_invalid,
	bw_hs2_coeffs_state_init,
	bw_hs2_coeffs_state_set_sample_rate,
	bw_hs2_coeffs_state_reset_coeffs
};
#endif
...
#ifdef BW_DEBUG_DEEP
	uint32_t			hash;
	enum bw_hs2_coeffs_state	state;
	uint32_t			reset_id;
#endif
...
#define BW_HS2_PARAM_CUTOFF	1
#define BW_HS2_PARAM_HIGH_GAIN	(1<<1)
...
static inline void bw_hs2_init(
	...
	coeffs->state = bw_hs2_coeffs_state_init;
...
#ifdef BW_DEBUG_DEEP
	coeffs->state = bw_hs2_coeffs_state_set_sample_rate;
#endif
	BW_ASSERT_DEEP(bw_hs2_coeffs_is_valid(coeffs));
	BW_ASSERT_DEEP(coeffs->state == bw_hs2_coeffs_state_set_sample_rate);
}

static inline void bw_hs2_update_mm2_params(
		bw_hs2_coeffs * BW_RESTRICT coeffs) {
...
#ifdef BW_DEBUG_DEEP
	coeffs->state = bw_hs2_coeffs_state_reset_coeffs;
	coeffs->reset_id++;
#endif
	BW_ASSERT_DEEP(bw_hs2_coeffs_is_valid(coeffs));
...
static inline void bw_hs2_reset_state_multi(
	...
	if (y_0 != BW_NULL)
		...
		for (size_t i = 0; i < n_channels; i++)
			bw_hs2_reset_state(coeffs, state[i], x_0[i]);
...
static inline char bw_hs2_coeffs_is_valid(
	...
	if (coeffs->hash != bw_hash_sdbm("bw_hs2_coeffs"))
		return 0;
	if (coeffs->state < bw_hs2_coeffs_state_init || coeffs->state > bw_hs2_coeffs_state_reset_coeffs)
		return 0;
#endif
...
#ifdef BW_DEBUG_DEEP
	if (coeffs->state >= bw_hs2_coeffs_state_reset_coeffs) {
		if (!bw_is_finite(coeffs->sg) || coeffs->sg < 0.f)
			return 0;
		if (!bw_is_finite(coeffs->ssg) || coeffs->ssg < 0.f)
			return 0;
	}
#endif
...
template<size_t N_CHANNELS>
inline void HS2<N_CHANNELS>::setSampleRate(
		float sampleRate) {
...
template<size_t N_CHANNELS>
inline void HS2<N_CHANNELS>::setQ(
		float value) {
	bw_hs2_set_Q(&coeffs, value);
}

template<size_t N_CHANNELS>
inline void HS2<N_CHANNELS>::setPrewarpAtCutoff(
		bool value) {
	bw_hs2_set_prewarp_at_cutoff(&coeffs, value);
}

template<size_t N_CHANNELS>
inline void HS2<N_CHANNELS>::setPrewarpFreq(
		float value) {
	bw_hs2_set_prewarp_freq(&coeffs, value);
}

template<size_t N_CHANNELS>
inline void HS2<N_CHANNELS>::setHighGainLin(
		float value) {
	bw_hs2_set_high_gain_lin(&coeffs, value);
}

template<size_t N_CHANNELS>
inline void HS2<N_CHANNELS>::setHighGainDB(
		float value) {
